...
const Services = () => {
  ...
  const filterPatients = (
    ...
    for (const key in data) {
      filteredData[key] = data[key].filter(
        (patient) => patient.type === 'IN_PERSON',
      )
    }
    ...
    <div className="w-full h-screen p-4 flex flex-col gap-4">
      ...
      <div className="bg-slate-100 rounded-xl flex w-12/12 h-full p-4 gap-4">
          ...
          <div className="flex items-center justify-between">
            <span
              className={cx(
                ...
                data &&
                  format(today, 'dd/MM') ===
                    format(parseISO(data?.startDate), 'dd/MM')
                  ? 'text-violet-500'
                  : 'text-zinc-400',
              )}
            >
            ...
            <span className="text-sm text-zinc-400">
              {data && format(parseISO(data?.startDate), 'dd/MM')}
            </span>
          </div>
          {!isLoading ? (
            <ScrollDayColumn>
              {patinents &&
                patinents[WeekDays.SEG].map((patient: any) => (
                  <PatientCard patient={patient} />
                ))}
              {patinents && patinents[WeekDays.SEG].length === 0 && (
                <EmptyColumn />
              )}
            </ScrollDayColumn>
          ) : (
        ...
        <div className="flex flex-col flex-1 gap-4">
          <div className="flex items-center justify-between">
              ...
              className={cx(
                ...
                data &&
                  format(new Date(), 'dd/MM') ===
                    format(addDays(parseISO(data?.startDate), 1), 'dd/MM')
                  ? 'text-violet-500'
                  : 'text-zinc-400',
              )}
            >
            ...
            <span className="text-sm text-zinc-400">
              {data && format(addDays(parseISO(data?.startDate), 1), 'dd/MM')}
            </span>
          </div>
          {!isLoading ? (
            <ScrollDayColumn>
              {patinents &&
                patinents[WeekDays.TER]?.map((patient: any) => (
                  <PatientCard patient={patient} />
                ))}
              {patinents && patinents[WeekDays.TER]?.length === 0 && (
                <EmptyColumn />
              )}
            </ScrollDayColumn>
          ) : (
          ...
          <div className="flex items-center justify-between">
            <span
              className={cx(
                ...
                data &&
                  format(new Date(), 'dd/MM') ===
                    format(addDays(parseISO(data?.startDate), 2), 'dd/MM')
                  ? 'text-violet-500'
                  : 'text-zinc-400',
              )}
            >
            ...
            <span className="text-sm text-zinc-400">
              {data && format(addDays(parseISO(data?.startDate), 2), 'dd/MM')}
            </span>
          </div>
          {!isLoading ? (
            <ScrollDayColumn>
              {patinents &&
                patinents[WeekDays.QUA].map((patient: any) => (
                  <PatientCard patient={patient} />
                ))}
              {patinents && patinents[WeekDays.QUA].length === 0 && (
                <EmptyColumn />
              )}
            </ScrollDayColumn>
          ) : (
          ...
          <div className="flex items-center justify-between">
            <span
              className={cx(
                ...
                data &&
                  format(new Date(), 'dd/MM') ===
                    format(addDays(parseISO(data?.startDate), 3), 'dd/MM')
                  ? 'text-violet-500'
                  : 'text-zinc-400',
              )}
            >
            ...
            <span className="text-sm text-zinc-400">
              {data && format(addDays(parseISO(data?.startDate), 3), 'dd/MM')}
            </span>
          </div>
          {!isLoading ? (
            <ScrollDayColumn>
              {patinents &&
                patinents[WeekDays.QUI].map((patient: any) => (
                  <PatientCard patient={patient} />
                ))}
              {patinents && patinents[WeekDays.QUI].length === 0 && (
                <EmptyColumn />
              )}
            </ScrollDayColumn>
          ) : (
          ...
          <div className="flex items-center justify-between">
            <span
              className={cx(
                ...
                data &&
                  format(new Date(), 'dd/MM') ===
                    format(addDays(parseISO(data?.startDate), 4), 'dd/MM')
                  ? 'text-violet-500'
                  : 'text-zinc-400',
              )}
            >
            ...
            <span className="text-sm text-zinc-400">
              {data && format(addDays(parseISO(data?.startDate), 4), 'dd/MM')}
            </span>
          </div>
          {!isLoading ? (
            <ScrollDayColumn>
              {patinents &&
                patinents[WeekDays.SEX].map((patient: any) => (
                  <PatientCard patient={patient} />
                ))}
              {patinents && patinents[WeekDays.SEX].length === 0 && (
                <EmptyColumn />
              )}
            </ScrollDayColumn>
          ) : (
          ...
          <div className="flex items-center justify-between">
            <span
              className={cx(
                ...
                data &&
                  format(new Date(), 'dd/MM') ===
                    format(addDays(parseISO(data?.startDate), 5), 'dd/MM')
                  ? 'text-violet-500'
                  : 'text-zinc-400',
              )}
            >
            ...
            <span className="text-sm text-zinc-400">
              {data && format(addDays(parseISO(data?.startDate), 5), 'dd/MM')}
            </span>
          </div>
          {!isLoading ? (
            <ScrollDayColumn>
              {patinents &&
                patinents[WeekDays.SAB].map((patient: any) => (
                  <PatientCard patient={patient} />
                ))}
              {patinents && patinents[WeekDays.SAB].length === 0 && (
                <EmptyColumn />
              )}
            </ScrollDayColumn>
          ) : (
          ...
          <div className="flex items-center justify-between">
            <span
              className={cx(
                ...
                data &&
                  format(new Date(), 'dd/MM') ===
                    format(addDays(parseISO(data?.startDate), 6), 'dd/MM')
                  ? 'text-violet-500'
                  : 'text-zinc-400',
              )}
            >
            ...
            <span className="text-sm text-zinc-400">
              {data && format(addDays(parseISO(data?.startDate), 6), 'dd/MM')}
            </span>
          </div>
          {!isLoading ? (
            <ScrollDayColumn>
              {patinents &&
                patinents[WeekDays.DOM].map((patient: any) => (
                  <PatientCard patient={patient} />
                ))}
              {patinents && patinents[WeekDays.DOM].length === 0 && (
                <EmptyColumn />
              )}
            </ScrollDayColumn>
          ) : (
      ...
      {isAddConsultModalOpen && (
        <AddPatientToCalendar
          ...
          setIsOpen={setIsAddConsultModalOpen}
        />
      )}
      {/* {} */}
    </div>
