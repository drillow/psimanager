...
export const PatientRow: React.FC<PatientRowProps> = ({ data }) => {
  ...
  return (
    <TableRow key={data.id}>
      <TableCell className="text-zinc-500 text-sm font-semibold w-[95px] antialiased">
        {data.patientId}
      </TableCell>
      <TableCell className="text-zinc-700 text-sm font-bold antialiased w-[250px]">{`${data.firstName} ${data.lastName}`}</TableCell>
      <TableCell className="text-zinc-500 text-sm font-semibold antialiased w-[175px]">
        {data.age}
      </TableCell>
      ...
      <TableCell className="text-center">
        {data.isWhatsApp ? (
          ...
          <Badge
            variant={'destructive'}
            className="bg-red-200 text-red-600 hover:bg-red-200 antialiased shadow-none"
          >
            Não
          </Badge>
        )}
