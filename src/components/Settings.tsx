...
import { CreditCard, KeyRound, Sparkles, User } from 'lucide-react'
    ...
    <Dialog>
      ...
      <DialogContent className="min-w-[896px] grid grid-cols-12 p-0 gap-0">
          ...
          <div className="flex flex-col justify-between h-full">
            <div className="flex flex-col">
              <div
                ...
                onClick={() => setSelectedMenu(MenuItens.PERSONAL)}
              >
                <User
                  className={cx(
                    `w-4 h-4`,
                    selectedMenu === MenuItens.PERSONAL && 'text-violet-500',
                  )}
                />
                <span
                  className={cx(
              ...
              <div
                className="flex items-center gap-2 py-2 cursor-pointer"
                onClick={() => setSelectedMenu(MenuItens.ACCOUNT)}
              >
                <KeyRound
                  className={cx(`w-4 h-4`, selectedMenu === MenuItens.ACCOUNT && 'text-violet-500')}
                />
                <span
                  className={cx('text-sm', selectedMenu === MenuItens.ACCOUNT && 'text-violet-500')}
                >
                  Dados da conta
                </span>
              </div>
              <div
                className="flex items-center gap-2 py-2 cursor-pointer"
                onClick={() => setSelectedMenu(MenuItens.FINANCE)}
              >
                <CreditCard
                  className={cx(`w-4 h-4`, selectedMenu === MenuItens.FINANCE && 'text-violet-500')}
                />
                <span
                  className={cx('text-sm', selectedMenu === MenuItens.FINANCE && 'text-violet-500')}
                >
                  Financeiro
                </span>
              </div>
        ...
        <div className="col-start-4 col-end-13 flex flex-col">
          {selectedMenu === MenuItens.PERSONAL && <PersonalForm />}

          {selectedMenu === MenuItens.ACCOUNT && <UserInfoForm />}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-2">
                      ...
                      <span className="text-base font-semibold">
                        {status === SubscriptionStatus.ACTIVE ? 'R$ 29.99/mês' : 'R$ 0.00/mês'}
                      </span>
                    </div>
                    <div className="flex items-center gap-4">
                      {status === SubscriptionStatus.ACTIVE ? (
                        <Button
                          variant={'link'}
                          onClick={() => handleCreateStripePortal(user.id)}
                          disabled={isLoading}
                        >
