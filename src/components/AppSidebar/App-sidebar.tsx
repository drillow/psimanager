...
import { CalendarDays, Settings, Users, LayoutDashboard, Headphones, Hexagon, DollarSign, Crown, ChartColumnBig } from 'lucide-react'
  ...
  return (
    <Sidebar className="bg-white" collapsible="icon">
      <SidebarHeader className="flex flex-row w-full items-center">
        <div className={`${open ? "w-8 h-8" : "w-7 h-7"} bg-violet-600 rounded-md flex items-center justify-center`}>
                  ...
                  <SidebarMenuButton asChild disabled>
                    {item.disabled ? (
                      <div className="cursor-pointer text-zinc-500">
                        <item.icon />
                        <span className="flex items-center gap-2">{item.title}
        ...
        {open && (
          <div className="flex flex-col border border-zinc-200 p-2 rounded-md mb-4">
            <div className="flex flex-col gap-2 mb-4">
              ...
              <span className="text-xs text-zinc-400">Seu plano <strong>gratuíto</strong> permite adicionar até <strong>5</strong> pacientes.</span>
            </div>
            <Progress value={20}/>
